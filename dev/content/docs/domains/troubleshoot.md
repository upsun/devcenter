...
including when you use a [CDN](/docs/domains/cdn/_index.md).
...
Often the problem is caused by a mismatch between the certificate and the domain name or an expired [custom certificate](/docs/domains/steps/tls.md).
...
If you have a [default domain](/docs/define-routes/_index.md#default) set up, the generated URL has the following pattern:
...
The certificate request is generated based on your [routes definition](/docs/define-routes/_index.md).
...
{{< tabs items="Without a CDN,Using a CDN" >}}
...
For more information, see how to [setup your CDN](/docs/domains/cdn/_index.md).
...
Make sure that the [apex domain](/docs/glossary.md#apex-domain) and its `www` subdomain are both pointing where needed.
Note that it can take up to 72 hours for DNS changes to be effective.
For more information, see how to [set up a custom domain](/docs/domains/steps/_index.md).

If the changes take longer than expected,
[redeploy](/docs/development/troubleshoot.md#force-a-redeploy) the impacted environment.
...
If not, [contact Support](/docs/learn/overview/get-support.md).

### Check your routes configuration

Certificates are generated based on your [routes configuration](/docs/define-routes/_index.md).
...
For more general information, see how to [troubleshoot development](/docs/development/troubleshoot).

If your website is still not working as expected, [contact support](/docs/learn/overview/get-support.md).
